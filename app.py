...
def create_app(db_url=None):
    ...
    def invalid_token_callback(error):
        return(
            jsonify({"message": "Signature verifiacation failed.", "error": "invalid_token"}), 401,
        )
    
    @jwt.unauthorized_loader
    def missing_token_callback(error):
        return(
            jsonify({"description":"Request does not contain an access token", "error": "token required"}),
            401,
        )
